...
def settings():
    ...
    user_id = st.session_state.get("username")  # you use this as user_id across tables
    ...
    with tab1:
        ...
        current_lang = (st.session_state.get("language") or "no").strip().lower()
        ...
        if st.button(
            "Save language",
            type="primary",
            key="settings_save_language_btn",
            width="stretch",
        ):
...
# ---- TRANSLATE DEFAULTS (SAFE, OPTIONAL) ----
with st.expander("🌍 Translate my default categories & default account", expanded=False):
    st.caption(
        "This will rename ONLY Ziva’s standard onboarding categories and your default account "
    ...
    if st.button(
        ...
        try:
            ...
            cat_n, acc_n = translate_defaults_for_user(
                user_id=user_id,
                target_lang=target_lang,
            )
            st.success(
                f"✅ Translation complete. Categories updated: {cat_n}, "
                f"default account updated: {acc_n}"
            )
            st.rerun()
        except Exception as e:
    ...
    # ==========================================
    # 3) SYSTEM / DATA (B2C TOOLS + DANGER ZONE)
    # ==========================================
    with tab3:
        st.subheader("Account & Data")

        # --------------------------
        # A) CHANGE PASSWORD (B2C)
        # --------------------------
        ...
        with st.expander("Export CSV (transactions, accounts, categories…)", expanded=False):
            ...
            with c1:
                st.download_button(
                    "⬇️ Transactions CSV",
                    data=_csv_bytes(tx),
                    file_name="ziva_transactions.csv",
                    mime="text/csv",
                    width="stretch",
                )
                st.download_button(
            ...
            with c3:
                st.download_button(
                    "⬇️ Loans CSV",
                    data=_csv_bytes(loans),
                    file_name="ziva_loans.csv",
                    mime="text/csv",
                    width="stretch",
                )
        ...
        # --------------------------
        # C) FACTORY RESET (KEEP YOUR USER)
        # --------------------------
        ...
        with st.expander("💣 Factory Reset (delete data, keep account)", expanded=False):
            confirm_text = st.text_input(
                ...
                key="settings_factory_reset_confirm_final",
            )
            if st.button(
                "Confirm Reset",
                type="primary",
        ...
        # --------------------------
        # D) DELETE ACCOUNT (DELETE USER + DATA)
        # --------------------------
        st.markdown("### 🧨 Delete account")
        with st.expander("Delete my account permanently", expanded=False):
            st.warning(
                "This will delete your user profile AND all your data (transactions, accounts, categories, budgets, payees…). "
            ...
            confirm_b = st.text_input(
                ...
                value="",
                key="settings_delete_account_confirm_b",
            ).strip().lower()
            ...
            if st.button("Delete account permanently", type="primary", disabled=not delete_ok, width="stretch"):
                ...
                try:
                    ...
                    # Delete the user row (username is primary key in your schema)
