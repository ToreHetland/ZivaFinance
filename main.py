...
# 4. SUBSCRIPTION GATEKEEPER (COMMENTED OUT - ALL USERS GET ACCESS)
   ...
   # --- Navigation State Logic (Fixed) ---
    ...
    # main.py accepts that change instead of overwriting it.
    st.session_state["nav_page"] = st.session_state["active_tab"]

    # 3. Sync legacy keys just in case other modules rely on them
    for _k in ("selected_page", "page", "current_page", "menu"):
        st.session_state[_k] = st.session_state["active_tab"]
